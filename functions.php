<?php
			...
			if ( $item_title == 'Контакты' ) {
				$output .= '
					<li class="nav-item d-none">
						<span class="nav-link">
							<img src="'.get_stylesheet_directory_uri().'/img/ico/menu-point.svg" alt="">
						</span>
					</li>
				';
	...
	add_filter( 'woocommerce_breadcrumb_defaults', 'jk_woocommerce_breadcrumbs_2', 20 );
	function jk_woocommerce_breadcrumbs_2() {
		return array(
				'delimiter'   => ' &#47; ',
				'wrap_before' => '<nav class="woocommerce-breadcrumb" itemprop="breadcrumb"><a href="/"><img src="'.get_stylesheet_directory_uri().'/img/ico/breadcrumbs-icon.svg"></a> / ',
				'wrap_after'  => '</nav>',
				'before'      => '',
				'after'       => '',
	...
	add_filter( 'woocommerce_breadcrumb_defaults', 'wcc_change_breadcrumb_home_text_2', 20 );
	function wcc_change_breadcrumb_home_text_2( $defaults ) {
	...
	add_filter( 'woocommerce_get_breadcrumb', 'my_woocommerce_get_breadcrumb_2' );
	function my_woocommerce_get_breadcrumb_2($breadcrumb) {
			...
			foreach ( $breadcrumb as $key => $crumb ) {
				...
				if ($breadcrumb[$key][0] == 'Наша продукция') $breadcrumb[$key][0] = 'Каталог';
			}
	...
	*/
	...
	/*** ИСКЛЮЧАЕМ ТОВАРЫ С ОПРЕДЕЛЕННЫМ ТЕГОМ ***/
	/*** В данный момент эта функция отключена, т.к. с ней неправильно работает виджет филтра по атрибутам товаров ***/
	/*** Со страницы архива всех тваров ***
	function exclude_products_with_tag( $query ) {
		// Проверяем, что это главный запрос и запрос товаров
		if ( ! is_admin() && $query->is_main_query() && is_post_type_archive( 'product' ) ) {
			// Укажите slug тега, который нужно исключить
			$tag_slug = 'portfolio'; // Замените на slug вашего тега

			// Получаем ID тега
			$tag = get_term_by('slug', $tag_slug, 'product_tag');
			if ( $tag ) {
				// Исключаем товары с этим тегом
	...
	add_action( 'pre_get_posts', 'exclude_products_with_tag' );
	
	
	/*** Со страницы архива таваров определенной категории ***
	function exclude_products_with_tag_from_category( $query ) {
		// Проверяем, что это не админка, главный запрос и страница категории товаров
		if ( ! is_admin() && $query->is_main_query() && is_product_category() ) {
			...
			if ( $tag ) {
				// Исключаем товары с этим тегом
				$tax_query = $query->get('tax_query'); // Получаем текущий tax_query
				if ( ! is_array( $tax_query ) ) {
					$tax_query = array();
				}

				// Добавляем условие для исключения товаров с указанным тегом
				$tax_query[] = array(
					'taxonomy' => 'product_tag',
					'field'    => 'term_id',
					'terms'    => array( $tag->term_id ),
				...
				// Устанавливаем обновленный tax_query
				$query->set( 'tax_query', $tax_query );
			}
		}
	}
	add_action( 'pre_get_posts', 'exclude_products_with_tag_from_category' );
	/*** END ИСКЛЮЧАЕМ ТОВАРЫ С ОПРЕДЕЛЕННЫМ ТЕГОМ ***/
