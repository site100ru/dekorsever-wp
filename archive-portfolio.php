	...
	function galleryOn(gal, img) {
		...
		<?php
			...
			foreach( $posts as $post ) {
				...
				$count2 = 0;
				for ( $i=1; $i<=9; $i++ ) {
					echo 'if(document.getElementById("img-'.$post->ID.'-'.$count2.'")) document.getElementById("img-'.$post->ID.'-'.$count2.'").classList.remove("active");';
					echo 'if(document.getElementById("ind-'.$post->ID.'-'.$count2.'")) document.getElementById("ind-'.$post->ID.'-'.$count2.'").classList.remove("active");';
					$count2 = $count2 + 1;
				}
			} wp_reset_postdata();
		?>
